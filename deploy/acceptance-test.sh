...
for (const required of [
  ...
  'src/portal-command-store.js','src/request-rate-limiter.js','src/approval-store.js','src/central-operation-guard.js',
  ...
  'test/request-rate-limiter.test.js','test/ticket-projection-store.test.js','test/sso-frontchannel-logout.test.js',
  'test/backup-archive.test.js','test/restore-transaction.test.js','test/updater-client-security.test.js',
  'test/e2e/ui-buttons.spec.js','docs/SECURITY-AUDIT-2026-07-31.md'
...
compose config >/tmp/sirk-central-compose-acceptance.yml
compose config --format json >/tmp/sirk-central-compose-acceptance.json
node - <<'NODE'
const fs = require('node:fs');
const compose = JSON.parse(fs.readFileSync('/tmp/sirk-central-compose-acceptance.json', 'utf8'));
...
if [[ "$SKIP_BUILD" != "true" ]]; then
  ...
  compose build central auth updater backup-manager
fi
...
updater_image="$(docker inspect "$updater_id" --format '{{.Image}}')"
...
[[ "$(docker image inspect "$updater_image" --format '{{.Config.User}}')" == "root" ]] || fail "Updater image root trust boundary is not explicit."
...
[[ "$(docker inspect "$updater_id" --format '{{.Config.User}}')" == "0:0" ]] || fail "Updater container must run as explicit root because Docker socket and repository update access are its documented trust boundary."
...
if [[ "$SKIP_LIVE" != "true" && -n "$PUBLIC_URL" ]]; then
  ...
  trap 'rm -f "$ready_headers" "$ready_body" "$root_headers" "$internal_body"' EXIT
