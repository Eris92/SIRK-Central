...
capture_safety_state() {
  ...
  if compose ps --services | grep -qx central; then
    ...
    fail "Unable to capture the current Central data before restore."
  fi
...
rollback() {
  ...
  trap - ERR
  set +e
  log "Restore failed; rolling back to the captured safety state"
  cd "${INSTALL_DIR}" || return 1
  compose stop central auth updater backup-manager >/dev/null 2>&1 || true
  cp -- "${SAFETY_DIR}/.env" .env
  chmod 0600 .env
  local safety_commit
  safety_commit="$(tr -d '[:space:]' < "${SAFETY_DIR}/commit.txt")"
  git reset --hard "${safety_commit}" || return 1
  [[ -f docker-compose.portal-runtime.yml ]] || return 1
  compose build central auth updater backup-manager || return 1
  replace_volume_data "${SAFETY_DIR}/data" || return 1
  compose up -d --force-recreate central auth updater backup-manager caddy || return 1
  wait_ready || return 1
...
if [[ -f "${CHECKSUM_FILE}" ]]; then
  ...
  printf 'WARNING: restoring a legacy archive without a checksum.\n' >&2
fi
...
log "Validating and safely extracting the archive"
...
log "Fetching and validating the target commit before stopping services"
git fetch --prune origin
git cat-file -e "${TARGET_COMMIT}^{commit}" || fail "Backup commit is unavailable."
for required_path in Dockerfile.portal-runtime docker-compose.yml docker-compose.portal-runtime.yml src/server-v15.js; do
  git cat-file -e "${TARGET_COMMIT}:${required_path}" || fail "Backup commit does not contain canonical runtime file: ${required_path}"
done
...
compose stop central auth updater backup-manager || true

log "Restoring repository and configuration"
cp -- "${BACKUP_DIR}/.env" .env
chmod 0600 .env
git reset --hard "${TARGET_COMMIT}"
[[ -f docker-compose.portal-runtime.yml ]] || fail "Target commit is missing the runtime overlay."
...
compose up -d --force-recreate central auth updater backup-manager caddy
wait_ready || fail "Restored Central did not become healthy."

ROLLBACK_ARMED=false
log "Restore completed"
printf 'Commit=%s\n' "${TARGET_COMMIT}"
printf 'Safety state was validated and removed after successful restore.\n'
