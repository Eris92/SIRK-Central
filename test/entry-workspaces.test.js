...
test("workspace matrix separates Admin, SysAdmin, SecAdmin and built-in Break-Glass", () => {
    const admin = { ok: true, source: "entra", role: "Admin", builtIn: false };
    const sysAdmin = { ok: true, source: "entra", role: "SysAdmin", builtIn: false };
    ...
    assert.deepEqual(allowedWorkspaces(admin), ["portals", "permissions", "settings"]);
    assert.deepEqual(allowedWorkspaces(sysAdmin), ["portals", "permissions", "settings"]);
    assert.deepEqual(allowedWorkspaces(secAdmin), ["portals", "security", "settings"]);
    assert.deepEqual(allowedWorkspaces(breakGlass), ["portals", "permissions", "security", "settings", "break-glass"]);
