...
function create(options = {}) {
    ...
    try {
        ...
        if (!parsed || parsed.schema !== 2 || !parsed.tickets || typeof parsed.tickets !== "object") {
            throw storeError("Unsupported ticket projection store schema.", "TICKET_STORE_SCHEMA_UNSUPPORTED", 500);
        }
        state = {
            schema: 2,
        ...
        for (const ticket of Object.values(state.tickets)) {
            if (!ticket.sourceUpdatedAtUtc || !ticket.sourceHash || !ticket.central || typeof ticket.central !== "object") {
                throw storeError("Ticket projection store requires schema 2 normalized records.", "TICKET_STORE_RECORD_UNSUPPORTED", 500);
            }
        }
    ...
    function normalize(portalId, input, previous, context = {}) {
        ...
        const ticketId = identifier(input.ticketId, "Ticket ID");
    ...
    function upsert(portalId, input, context = {}) {
        const ticketKey = key(portalId, input && (input.ticketId));
    ...
    function snapshot(portalId, input, context = {}) {
        ...
        for (const rawItem of items) {
            const ticketKey = key(portalId, rawItem && rawItem.ticketId);
    ...
    function event(portalId, input, context = {}) {
        ...
        if (events[eventId]) {
            ...
            const ticketId = input.ticket && input.ticket.ticketId;
        ...
        const ticketKey = key(portalId, rawTicket.ticketId);
        let ticket = null;
        let accepted = false;
        let stale = false;
        let duplicate = false;
