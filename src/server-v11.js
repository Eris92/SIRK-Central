...
function canWrite(actor) {
    return Boolean(actor && (actor.builtIn === true || ["Admin", "SecAdmin"].includes(actor.role)));
}
...
function managerOrigin(config) {
    return String(config.env.SIRK_BACKUP_MANAGER_ORIGIN || "http://backup-manager:8091").replace(/\/+$/, "");
}
    ...
    const server = http.createServer(async (req, res) => {
        try {
            ...
            if (req.method === "GET" && url.pathname === "/api/audit/export") {
                if (!canRead(actor) || !app.auditStore) return json(res, 403, { ok: false, error: "Permission denied." });
                ...
                if (format === "json") {
                    ...
                    res.writeHead(200, { "Content-Type": "application/json; charset=utf-8", "Content-Length": String(data.length), "Content-Disposition": "attachment; filename=central-audit.json", "Cache-Control": "no-store" });
                    return res.end(data);
                }
                ...
                res.writeHead(200, { "Content-Type": "text/csv; charset=utf-8", "Content-Length": String(data.length), "Content-Disposition": "attachment; filename=central-audit.csv", "Cache-Control": "no-store" });
                return res.end(data);
            }
...
module.exports = { createAdminRuntime, VERSION };
