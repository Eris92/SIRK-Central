...
function sign(token, timestamp, nonce, rawBody) {
    return crypto.createHmac("sha256", String(token || "")).update(canonicalPayload(timestamp, nonce, rawBody), "utf8").digest("base64url");
}
...
function updaterState(input, capabilities) {
    const installed = capabilities.includes("shared-updater") || input.updaterInstalled === true;
    const running = capabilities.includes("shared-updater-running") || input.updaterRunning === true;
    const phaseCapability = capabilities.find(value => value.startsWith("shared-updater-phase:"));
    const phase = cleanText(input.updaterPhase || (phaseCapability ? phaseCapability.slice("shared-updater-phase:".length) : ""), 80);
    let status = "not-installed";
    if (installed && !running) status = "stopped";
    if (installed && running) status = phase && !["completed", "failed"].includes(phase) ? "busy" : "ready";
    if (phase === "failed") status = "failed";
    return {
        installed,
        running,
        status,
        channel: cleanText(input.updaterChannel || input.updateChannel, 80),
        targetVersion: cleanText(input.updaterTargetVersion || input.availableVersion, 80),
        phase,
        updatedAtUtc: optionalIso(input.updaterUpdatedAtUtc)
    };
}
function normalizeHeartbeat(input) {
    ...
    const capabilities = Array.isArray(input.capabilities)
        ...
        : [];
    const updater = updaterState(input, capabilities);
    return {
...
function create(options) {
    ...
    function accept(portal, envelope) {
        ...
        const activeNonces = (Array.isArray(previous.nonces) ? previous.nonces : []).filter(item => item && Number(item.expiresAt) > currentTime && typeof item.value === "string");
    ...
    function list(registry) {
        ...
        return [...ids].sort().map(id => {
            ...
            return { id, name: registered && registered.name || id, registered: Boolean(registered), status: "never", ageSeconds: null,
                firstSeenAtUtc: null, lastSeenAtUtc: null, lastRemoteTimestampUtc: null, heartbeatCount: 0, metrics: normalizeHeartbeat({}) };
        });
    }
    function remove(id) {
        const key = String(id || "").toLowerCase();
        if (!state.portals[key]) return false;
        delete state.portals[key]; persist(); return true;
    }
    return { accept, get, list, remove, filePath, onlineAfterMs, maximumClockSkewMs, maxNoncesPerPortal };
}
module.exports = { create, sign, verifySignature, canonicalPayload, normalizeHeartbeat, cleanPublicUrl, optionalIso, updaterState };
