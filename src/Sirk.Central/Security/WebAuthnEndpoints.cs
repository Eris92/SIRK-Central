...
internal static class WebAuthnEndpoints
{
    ...
    public static IEndpointRouteBuilder MapSirkWebAuthn(this IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.MapPost("/api/v1/break-glass/{accessCode}/webauthn/options", AssertionOptions)
            .AllowAnonymous()
        ...
        WebAuthnCeremonyStore ceremonies)
    {
        ...
        var excluded = store.ListByUser(userId)
            .Select(value => new PublicKeyCredentialDescriptor(WebAuthnCredentialStore.Decode(value.CredentialId)))
            .ToList();
        ...
        SecurityAuditLog audit)
    {
        ...
        try
        {
            ...
            IsCredentialIdUniqueToUserAsyncDelegate unique = (args, _) =>
                Task.FromResult(!store.Exists(args.CredentialId));
    ...
    private static IResult AssertionOptions(
        string accessCode,
        WebAuthnAssertionOptionsRequest request,
        LocalIdentityStore identities,
        IFido2 fido2,
        WebAuthnCredentialStore store,
        WebAuthnCeremonyStore ceremonies)
    {
        ...
        IOptions<SecurityOptions> securityOptions)
    {
        ...
        try
        {
            var options = AssertionOptions.FromJson(ceremonies.Consume(request.CeremonyId, "assertion", identity.Id));
            var credential = store.Get(request.Response.Id)
            ...
            var claims = new List<Claim>
            {
                ...
                new(ClaimTypes.Role, SirkRoles.BreakGlass),
                new("amr", "webauthn"),
    ...
    private static string UserHandle(string userId)
    {
        ...
        return WebAuthnCredentialStore.Base64Url(System.Security.Cryptography.SHA256.HashData(bytes));
    }
    ...
    private static IResult CsrfFailure() =>
        Results.Json(new { ok = false, code = "CSRF_VALIDATION_FAILED" }, statusCode: 400);
