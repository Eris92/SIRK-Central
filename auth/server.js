...
function loadConfig(env) {
    ...
    return {
        authOrigin, centralOrigin, sharedSecret,
        port: Number(env.SIRK_AUTH_PORT || 8081),
...
function createApp(config) {
    ...
    function provider() {
        ...
        if (!value.enabled) throw new Error("Microsoft Entra login is disabled in SIRK Central.");
        if (!value.clientId || !value.clientSecret) throw new Error("Microsoft Entra configuration is incomplete.");
        return value;
    }
    ...
    async function getJwk(kid) {
        if (jwksCache.expiresAt < Date.now()) {
            const response = await fetch("https://login.microsoftonline.com/common/discovery/v2.0/keys", { headers:{accept:"application/json"} });
            if (!response.ok) throw new Error("Unable to download Entra signing keys.");
            const body = await response.json(); jwksCache = { expiresAt: Date.now() + 3600000, keys: Array.isArray(body.keys) ? body.keys : [] };
        }
        const key = jwksCache.keys.find(item => item.kid === kid && item.kty === "RSA");
        if (!key) { jwksCache.expiresAt = 0; throw new Error("Entra signing key was not found."); }
        return key;
    }
